...
class readData():
    ...
    # 读取 sqlite 表名列表
    def _read_sqlite_tablenames(self):
        import sqlite3
        import pandas as pd

        db = self._config.get('sqlite', 'db')
        db_conn = sqlite3.connect(db)
        get_tableNames_sql = """SELECT name FROM sqlite_master where type='table' order by name """
        tablenames = pd.read_sql(get_tableNames_sql, db_conn).iloc[:, 0].tolist()
        db_conn.close()
        return tablenames

    # 读取 sqlite 表数据
    def _read_sqlite_data(self, tablename):
        import sqlite3
        import pandas as pd
        ...
        db_conn = sqlite3.connect(db)
        get_data_sql = """select * from %s""" % tablename
        data = pd.read_sql(get_data_sql, db_conn)
        db_conn.close()
        return data
    ...
    def read_tablenames(self):
        ...
        elif self._config['savetype']['type'] == 'sqlite':
            return self._read_sqlite_tablenames()
    ...
    def read_data(self, tablename):
        if self._config['savetype']['type'] == 'mysql':
            ...
            return self._read_sqlite_data(tablename)