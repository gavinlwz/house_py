...
class saveData():
    ...
    def _delete_sqlite(self):
        ...
        tablename = 'T' + timestring + 'TheFutureOfHome'
        drop_sql = """drop table IF EXISTS %s""" % (tablename)
        drop_rows = db_conn.execute(drop_sql)
        print('delete ' + str(drop_rows) + ' rows.')

        db_conn.commit()
        db_conn.close()

    def _save_sqlite(self, webName, houseName, villageName, houseNote, houseTotlePrice, houseUnitPrice, houseLink,
                    houseImg):
        import sqlite3
        db = self._config.get('sqlite', 'db')
        db_conn = sqlite3.connect(db)
        ...
        if len(houseName) > 0:
            try:
                saved_rows = db_conn.execute(insert_sql)
            except:
                print(insert_sql)
        print(webName + ' saved ' + str(saved_rows) + ' rows.')
        db_conn.commit()
